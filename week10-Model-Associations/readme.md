...
This considers that you have finished with the last lab exercises and have a back-end with mongoose models to start with. 
...
Make sure you have your application up and running before starting.
...
Notice the attribute `animals` is a reference to the `animal` model created previously.
...
This can be done directly in the back-end when retrieving your user or from the front-end by explicitly making calls to the back-end to retrieve the corresponding animal.
...
This was the approach adopted in the sample front-end solutions (both react and vue).
...
1. Finish the implementation of the retrive and update operations with the `User` model (both back-end and front-end using your chosen technology).
...
5. Reflect on the new API provided by your back-end. Does it follow the best practices? How would you change it to do so?
